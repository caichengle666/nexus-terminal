export const CONFIG_KEY = 'nexus_ai_terminal_config';
export const MEMORIES_KEY = 'nexus_ai_terminal_session_memories';
export const LEGACY_MESSAGES_KEY = 'nexus_ai_terminal_messages';
export const LEGACY_TOOL_RUNS_KEY = 'nexus_ai_terminal_tool_runs';
export const MAX_SAVED_MESSAGES = 120;
export const MAX_SAVED_TOOL_RUNS = 80;
export const MAX_SAVED_MEMORY_BYTES = 4 * 1024 * 1024;
export const MAX_IMPORT_FILE_BYTES = 5 * 1024 * 1024;
export const MAX_SAVED_CONTENT_LENGTH = 24000;
export const MAX_MODEL_RECENT_MESSAGES = 16;
export const DEFAULT_AUTO_COMPACTS_PER_TASK = 2;
export const MIN_AUTO_COMPACTS_PER_TASK = 1;
export const MAX_AUTO_COMPACTS_PER_TASK = 5;
export const MAX_COMPACT_TOOL_RUNS = 40;
export const MAX_AI_REQUEST_BYTES = 256 * 1024;
export const MIN_AI_REQUEST_BYTES = 64 * 1024;
export const MAX_CONFIGURABLE_AI_REQUEST_BYTES = 1024 * 1024;
export const AI_REQUEST_COMPACT_BYTES = Math.floor(MAX_AI_REQUEST_BYTES * 0.8);
export const MIN_COMPACT_TRIGGER_PERCENT = 1;
export const MAX_COMPACT_TRIGGER_PERCENT = 80;
export const MAX_MODEL_CONTEXT_CHARS = 180000;
export const MAX_MODEL_MESSAGE_CONTENT_LENGTH = 65536;
export const MAX_MODEL_SUMMARY_LENGTH = 8000;
export const MAX_TERMINAL_OUTPUT_CHARS = 60000;
export const MAX_TOOL_RESULT_CONTENT_LENGTH = 64000;
export const MAX_TOOL_RESULT_SUMMARY_LENGTH = 60000;
export const TAIL_CONTEXT_BUDGET_CHARS = 131072;
export const MIN_TAIL_MESSAGES = 3;
export const DEFAULT_TERMINAL_READ_LINES = 800;
export const MAX_TERMINAL_READ_LINES = 3000;

export const SUMMARY_SECTION_TITLES = ['本次压缩摘要', '手动压缩摘要', 'AI 智能摘要'];
